    ...
    <nav className="fixed top-0 left-0 w-full z-50 bg-black/80 backdrop-blur-md border-b border-white/5">
      <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 min-h-20 py-3 flex flex-wrap items-center justify-between gap-3">
        ...
        <Link to="/" className="flex min-w-0 items-center gap-2 group">
          <div className="shrink-0 bg-[#f5c518] p-1.5 rounded-lg">
            <Film size={24} className="text-black" />
          </div>
          <span className="truncate text-xl sm:text-2xl font-black tracking-tighter text-white">
            CINE<span className="text-[#f5c518]">BOOK</span>
          </span>
        </Link>

        {/* Centered Navigation */}
        <div className="order-3 flex w-full items-center justify-start gap-6 overflow-x-auto border-t border-white/5 pt-3 sm:justify-center md:order-none md:w-auto md:border-t-0 md:pt-0 md:gap-10">
          <button
            onClick={() => {
              navigate("/");
          ...
          {user ? (
            <div className="flex min-w-0 flex-wrap items-center justify-end gap-2 sm:gap-3">
              {user.role === "admin" && (
                <Link
                  to="/admin"
                  className="btn-outline-gold !py-1.5 !px-3 sm:!px-4 text-[10px] tracking-widest whitespace-nowrap"
                >
                  ADMIN
                </Link>
              )}
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <Link
                to="/register"
                className="btn-fill-gold !py-1.5 !px-3 sm:!px-4 text-[10px] tracking-widest whitespace-nowrap"
              >
                JOIN
              </Link>
            </div>
